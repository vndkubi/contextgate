...
export const DEFAULT_CONFIG: TokenOptConfig = {
  ...
  context: {
    ...
    userPromptGuidance:
      "TokenOpt is active. When tokenopt MCP tools are available, treat natural repo prompts such as investigate, plan, implement, write tests, review, or build handoff as a signal to call tokenopt_compile_evidence first with the inferred task_type. If the packet is answerable, answer from it instead of replaying shell/search/file reads; otherwise use only bounded followups."
  },
  paths: {},
  codex: {
    installScope: "repo"
